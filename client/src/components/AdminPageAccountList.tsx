...
const AdminPageAccountList = () => {
  ...
  const getUserData = useCallback(() => {
    axios.get(`${import.meta.env.VITE_SERVER_URL}/admin/user`).then((res) => {
  ...
  const postDeleteUser = useCallback(() => {
    userList.forEach((item) => {
      if (item.checked) {
        axios
          .delete(
            `${import.meta.env.VITE_SERVER_URL}/admin/user?user_no=${
              item.user_no
            }`
          )
          .then((res) => {
  ...
  return (
    <AdminPageWrap>
      <h1>회원목록</h1>
      <ButtonBox>
        <Button onClick={postUpdateAuthority} width="80">
          수정
        </Button>
        <Button onClick={postDeleteUser} width="80">
          탈퇴
        </Button>
      </ButtonBox>
      <TableWrap>
                  ...
                  <tr key={item.user_no}>
                    ...
                    <td>
                      <Select
                        ...
                        onChange={(e) => {
                          setAuthority(e?.value as string);
                        }}
                      />
                    </td>
                  </tr>
